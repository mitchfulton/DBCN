...
class BayesDefNormReLU(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, network_props, conv, stride=None):
        """
        ...
        self.conv = conv(input_channels, output_channels, kernel_size,
...
class Bayes_deform_Layer(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, network_props, num_blocks, first_stride=None, layer_type='c'):
        super().__init__()
        ...
        if layer_type=='c':
            ...
            conv = BayesDeformConvPack
            num_blocks=1

        self.convs = nn.Sequential(
            BayesDefNormReLU(input_channels, output_channels, kernel_size, network_props, conv, first_stride),
            *[BayesDefNormReLU(output_channels, output_channels, kernel_size, network_props,nn.Conv3d) for _ in
              range(num_blocks - 1)]
        )
